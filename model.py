...
class VGGBase(nn.Module):
    ...
    def forward(self, x):
        ...
        Arguments:
            x {input image} -- images, a tensor of dimensions (N, 3, 300, 300)
            return: low-level feature maps conv4_3 and conv7
        """
    ...
    def load_pretrained_layers(self):
        ...
        # Pretrainned VGG base
        pretrained_state_dict = vgg16(pretrained=True).state_dict()
        pretrained_param_names = list(pretrained_state_dict.keys())
        from IPython import embed; embed()
        # Transfer conv. parameters from pretrained model to current model
        for i, param in enumerate(params_names[:-4]):  # excluding conv6 and conv7 params
